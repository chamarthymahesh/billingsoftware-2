...
const UserRoles = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const { data } = await axios.get('http://localhost:5000/api/users/sub-users', config);
      setUsers(data);
      setLoading(false);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.id) {
        ...
        await axios.put(`http://localhost:5000/api/users/sub-users/${formData.id}`, payload, config);
      } else {
        // Create
        await axios.post('http://localhost:5000/api/users/sub-users', formData, config);
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this user account?')) {
      try {
        const userInfo = JSON.parse(localStorage.getItem('userInfo'));
        const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
        await axios.delete(`http://localhost:5000/api/users/sub-users/${id}`, config);
        fetchUsers();
      } catch (err) {
                      ...
                      <div className="perms-list">
                        {(user.permissions && user.permissions.length > 0) ? 
                          user.permissions.map(p => (
                            <span key={p} className="perm-tag">{getLabelForPath(p)}</span>
                          )) 
                          : <span className="perm-tag" style={{ color: '#ef4444', background: 'rgba(239,68,68,0.1)' }}>No Access</span>
                        }
                      </div>
                    </td>
                    <td>
                      <div style={{ display: 'flex', gap: '12px' }}>
                        <button onClick={() => handleOpenModal(user)} style={{ background: 'transparent', border: 'none', color: '#f59e0b', cursor: 'pointer' }}>
                          <Edit size={18} />
                        </button>
                        <button onClick={() => handleDelete(user._id)} style={{ background: 'transparent', border: 'none', color: '#ef4444', cursor: 'pointer' }}>
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </td>
        ...
        <div className="modal-overlay">
          <div className="modal-content animate-slide-up">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                <label>Password {formData.id && <span style={{fontSize: '0.8rem', color: '#94a3b8'}}>(Leave blank to keep current)</span>}</label>
                <input 
                  type="password" 
              ...
              <div className="form-group" style={{ marginTop: '24px' }}>
                ...
                <p style={{ fontSize: '0.8rem', color: '#94a3b8', marginBottom: '12px' }}>Select which menus this user can access in their sidebar:</p>
              ...
              <div className="modal-actions">
                ...
                <button type="submit" className="save-btn">{formData.id ? 'Save Changes' : 'Create User'}</button>
              </div>
            </form>
          </div>
        </div>
