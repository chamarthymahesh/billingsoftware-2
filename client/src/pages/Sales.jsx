...
const Sales = () => {
  ...
  const totalSales = sales.reduce((sum, s) => sum + (s.grandTotal !== undefined ? s.grandTotal : Math.round((s.subtotal || 0) + (s.totalTax || 0))), 0);
  const paidSales = sales.filter(s => s.paymentStatus === 'Paid').reduce((sum, s) => sum + (s.grandTotal !== undefined ? s.grandTotal : Math.round((s.subtotal || 0) + (s.totalTax || 0))), 0);
            ...
            {loading ? (
              ...
              filtered.map(s => (
                <tr key={s._id}>
                  ...
                  <td className="sl-bold-price">₹{(s.grandTotal !== undefined ? s.grandTotal : Math.round((s.subtotal || 0) + (s.totalTax || 0))).toFixed(2)}</td>
                  <td>
