...
const Sales = () => {
  ...
  const filtered = sales.filter((s) => {
    ...
    const matchesProduct = s.items?.some((item) => (item.productName || "").toLowerCase().includes(searchLower));
    return matchesCustomer || matchesInvoice || matchesGem || matchesProduct;
  });

  const totalSales = sales.reduce(
    (sum, s) => sum + (s.grandTotal !== undefined ? s.grandTotal : Math.round((s.subtotal || 0) + (s.totalTax || 0))),
    0,
  );
  ...
  const getStatusClass = (status) => {
    switch (status) {
      case "Paid":
        return "sl-status-paid";
      case "Partial":
        return "sl-status-partial";
      default:
              ...
              filtered.map((s) => (
                <tr key={s._id}>
                  <td style={{ paddingRight: "0" }}>
                    <input
                      type="checkbox"
                      checked={selectedInvoices.includes(s._id)}
                  ...
                  <td>
                    <div className="sl-customer-name">{s.customerName || "-"}</div>
                  </td>
                  <td>{s.items?.length || 0} items</td>
                  <td>₹{(s.subtotal || 0).toFixed(2)}</td>
                  <td>₹{(s.totalTax || 0).toFixed(2)}</td>
                  <td className="sl-bold-price">
                    ₹
                    {(s.grandTotal !== undefined
                      ? s.grandTotal
                      : Math.round((s.subtotal || 0) + (s.totalTax || 0))
                    ).toFixed(2)}
                  </td>
                  <td>
                  ...
                  <td>
                    <div style={{ display: "flex", gap: "12px", justifyContent: "flex-start", minWidth: "80px" }}>
                      <button
                        ...
                        onClick={() => navigate(`/sales/view/${s._id}`)}
                        title="View"
                      >
                        <Eye size={18} />
                      </button>
                      ...
                      <button
                        ...
                        onClick={async () => {
                          if (
                            window.confirm(
                              "Are you sure you want to delete this invoice? This will restore the product stock levels.",
                            )
                          ) {
                            try {
